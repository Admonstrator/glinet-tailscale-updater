...
setup_usb_storage() {
    ...
    if mke2fs -t ext4 -L tailscale -F "$USB_DEV" >/dev/null 2>&1; then
        ...
        if [ "$EXISTING_INSTALL" -eq 1 ]; then
            echo "# Step 1: Stop and disable existing Tailscale service"
            echo "/etc/init.d/tailscale stop"
            echo "/etc/init.d/tailscale disable"
            echo "killall tailscaled"
            echo "sleep 2"
            echo ""
            echo "# Step 2: Remove old scripts and symlinks"
            echo "rm -f /etc/init.d/tailscale"
            echo "rm -f /etc/hotplug.d/block/10-mount-tailscale-usb"
            echo "rm -f /usr/sbin/tailscale"
            echo "rm -f /usr/sbin/tailscaled"
            echo ""
        fi

        echo "# Step 3: Force unmount the USB drive (lazy unmount)"
        echo "umount -l $USB_DEV 2>/dev/null || true"
        echo "umount -l $USB_MOUNT_POINT 2>/dev/null || true"
        echo ""
        echo "# Step 4: Clear kernel cache and write zeros to device"
        echo "sync"
        echo "dd if=/dev/zero of=$USB_DEV bs=1M count=10"
        echo "sync"
        echo "sleep 2"
        echo ""
        echo "# Step 5: Format the USB drive"
        echo "mke2fs -t ext4 -L tailscale -F $USB_DEV"
        echo ""
        echo "If it still fails after step 5, reboot the router and try again."
        echo ""
        echo "Once formatting succeeds, re-run this script with:"
